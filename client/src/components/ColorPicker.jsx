...
const ColorPicker = ({onChange , selectedColor}) => {
  ...
  return (
    <div className='relative'>
        ...
        <button onClick={()=> setisOpen(!isOpen)} className='flex items-center gap-1 text-sm text-purple-600 bg-gradient-to-br from-purple-100 ring-purple-300 hover:ring transition-all px-3 py-2 rounded-lg'>
            ...
            <div className='grid grid-cols-4 w-60 gap-2 absolute top-full left-0 right-0 p-3 mt-2 z-10 bg-white rounded-md border border-gray-200 shadow-sm'>
                ...
                {colors.map((color)=>(
                    <div key={color.value} className='relative cursor-pointer group flex flex-col' onClick={()=>{
                       ...
                       <p className='text-sm text-center mt-1 text-gray-600'>{color.name}</p>
                    </div>
                ))}
